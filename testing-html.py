...
def get_route():
    route_pairwise = zip(find_path()[:-1], find_path()[1:])
    route = [create_graph().loc[edge, 'geometry'].iloc[0] for edge in route_pairwise]
...
def make_map2():
    ...
    folium.GeoJson(get_route(), style_function=lambda x:route_style).add_to(mapRoute)

    target_stop = read_CAT()[read_CAT().closest_id == find_path()[-1]]
    target_union = target_stop.unary_union
    global name
    name = ""
    try:
        for t in target_union:
            name = read_CAT()[read_CAT()['geometry'] == t]['StopName'].to_numpy()[0]
        ...
        name = read_CAT()[read_CAT()['geometry'] == target_union]['StopName'].to_numpy()[0]
...
@st.cache
def map2():
    return make_map2().get_root().render()
