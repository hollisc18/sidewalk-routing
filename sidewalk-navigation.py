...
mapRoute = folium.Map(location = [addr_lat,addr_long], tiles = 'OpenStreetMap', zoom_start = 16)
...
route_zip = zip(short_path[:-1], short_path[1:])
route = [edges_gdf.loc[edge, 'geometry'].iloc[0] for edge in route_zip]
#https://www.py4u.net/discuss/155637

route_gdf = gpd.GeoDataFrame(route)
route_gdf.rename( columns={0 :'geometry'}, inplace=True)

route_style = {'fillColor': '#00A9ED', 'color': '#00A9ED', 'weight' : 7}
...
#if target union contains two stops (north bound and south bound) add both
try:
